...
void printMonth(int mon,int year,int x,int y){ //prints the month with all days
    ...
    if(!(mon>=1 && mon<=12)){
        printf("\n  INVALID MONTH\n\n");
        printf("  Press 'q' twice to return ......");
        getch();
        return;
    }
    if(!(year>=1600)){
    ...
    gotoxy(8, y+2);
    SetColor(14);
    printf("Press 'n' to Next month, Press 'p' to Previous month and 'q' to Quit");
    gotoxy(8,y+3);
    printf("Red Background indicates the Events, Press 's' to see them");
    ClearColor();
}
...
void AddNote(){
    ...
    gotoxy(5,6);
    printf("Enter a time frame(from time1 to time2): ");
    scanf("%d%d%d%d",&R.h1,&R.h2,   &R.h3,&R.h4);
    gotoxy(5,7);
    printf("Enter the location(city): ");
    fflush(stdin);
    scanf("%[^\n]",R.note1);
    gotoxy(5,8);
    printf("Additional info(max 50 characters): ");
    fflush(stdin);
    scanf("%[^\n]",R.note);
    if(R.h1>=0 && R.h1<=23 && R.h2>=0 && R.h2<=59 && R.h3>=0 && R.h3<=23 && R.h4>=0 && R.h4<=59){
        if(fwrite(&R,sizeof(R),1,fp)){
            gotoxy(5,13);
            puts("\tEvent is created !");
            fclose(fp);
        }else{
            gotoxy(5,12);
            SetColor(12);
...
void showNote(int mm,int yy){
    ...
    while(fread(&R,sizeof(R),1,fp) == 1){
        if(R.mm == mm && R.yy == yy){
            gotoxy(4,2+i);
            printf("Event from day %d at %d:%d o'clock is : %s , in %s\n",R.dd, R.h1,R.h2, R.note, R.note1);
            gotoxy(4,2+i);
            isFound = 1;
            i++;
        }
    ...
    if(isFound == 0){
        gotoxy(10,5);
        printf("This month has no events\n");
    }
    gotoxy(10,9+i);
    printf("Press any key to go back .......");
    getch();
...
void showEvents(int dd,int mm,int yy){
    FILE *fp;
    int i=1, isFound=0;
    fp = fopen("note.dat","rb");
    while(fread(&R,sizeof(R),1,fp) == 1){
        if(fp != NULL){
            gotoxy(2,2+i);
            printf("On %d.%d.%d from %d:%d o'clock to %d:%d o'clock : %s , in %s\n",R.dd,R.mm,R.yy ,R.h1,R.h2,R.h3,R.h4, R.note, R.note1);
            gotoxy(5,2+i);
            i++;
        }
        i++;
    ...
    if(fp == NULL){
        gotoxy(10,5+i);
        printf("There are no registered events\n");
    }
    gotoxy(10,8+i);
    printf("Press any key to go back ......");
    getch();
...
int main(){
    ...
    SetConsoleTitle("Calendar Project - Programming-Technique");
    int choice;
    char ch = 'a';
    while(1){
        system("cls");
        ...
        switch(choice){
            ...
            case 3 :
                ...
                break;
            case 4 :
                showEvents(date.dd , date.mm , date.yy);
                break;
            case 5 :
                exit(0);
        }
